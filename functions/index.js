...
//    Reject the API request and send an error to the sender if there was no dispenser with the corresponding dispenserId and companyId
  ...
  if (newUseCount <= limit) { // if the refill is not yet empty
...
//      Update the dispenser document in the dispensers collection
    ...
    for (let user of userData) { // for each user that is registered to this company
      ...
      if ( // if the refill is empty or is below the preset refill level threshold
        newUseCount == limit ||
...
//    much of the code is similar to the above code
  ...
  if (+level === 100) { // check if the refill is already full
    res.send("Refill is already full");
  } else {

    await db.collection("dispensers").doc(dispenserDocId).update({ // update the dispenser document with the new refill level
      useCount: 0,
      level: 100,
    });

    db.collection("usageData").add({ // add a new usageData document
      dispenserId,
      timeStamp: new Date().toISOString(),
      companyId,
      wasUsed: false,
    ...
    for (let user of userData) { // for each user that is registered to this company
      ...
      if (notifyWhenRefilled) { // send notification to them if they want to be notified
        try {
